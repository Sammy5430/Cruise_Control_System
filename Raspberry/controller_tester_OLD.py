...
class PiThread (threading.Thread):
    ...
    def run(self):
        ...
        if self.thread_name is "status":
            ...
            while True:
                if begin_test:
                    f = open("/home/pi/Documents/CCET/dynamicFalero_4_21_2021("+str(num_test)+").csv", 'a')
...
control_gain = 0.48                         # controller gain. Preliminary values from C Falero testing
...
def cruise_set_btn():
    ...
    if is_cruise_on:
        ...
        if 3 <= mph_val <= 7:    # if less than 3mph, can't activate
            ...
            set_spd.value = "{x:.1f} mph".format(x=cruise_set_spd)
            f = open("/home/pi/Documents/CCET/dynamicFalero_4_21_2021("+str(num_test)+").csv", 'w')
